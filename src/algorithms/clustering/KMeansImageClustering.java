...
public class KMeansImageClustering extends ImageProcessingTask {
     ...
     */
    public KMeansImageClustering(PlanarImage pInput,int numClusters,int maxIterations, double epsilon) {
        ...
        pOutput = new TiledImage(pInput.getMinX(),pInput.getMinY(),width,height, pInput.getMinX(),pInput.getMinY(),pInput.getSampleModel(),pInput.getColorModel());
        ...
        // Initialization with random values, using the maximum of the data values as limit.
    ...
    */
    private double[] getExtrema(PlanarImage image) {
        ...
        // We must get the extrema of all bands
   ...
   /**
    * This method  runs the classic K-Means clustering algorithm:
    ...
    */
    public void run() {
        ...
        for(iteration=0;iteration<maxIterations;iteration++) {
            ...
            // STEP 0 - Clean the cluster assignment vector
            Arrays.fill(clusterAssignmentCount,0);

            // STEP 1 - Scan the image and calculate the assignment vector
            ...
            // STEP 2 - Scan the assignment vector and recalculate the cluster centers
            for(int cluster=0;cluster<numClusters;cluster++) {
            ...
            // STEP 2a - Recalculate the centers.
            for(int cluster=0;cluster<numClusters;cluster++) {
            ...
            // STEP 3 - Calculate statistics and repeat from 1 if needed.
            sumOfDistances = 0;
            for(int h=0;h<height;h++) {
                for(int w=0;w<width;w++) {
                    ...
                    // Calculate the distance between this pixel's values and its assigned cluster center values.
                    double distance = 0;
                    int index = (h*width+w)*numBands;
                    for(int band=0;band<numBands;band++) {
        ...
        // Updates the status of the computation
        finished = true;
